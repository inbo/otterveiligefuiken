...
estimate_trend_pos <- function(
  ...
  assert_that(
    ...
    is.number(count_sd_waterlichaam), noNA(count_sd_waterlichaam),
    count_sd_waterlichaam > 0,
  ...
  query <- sprintf(
   ...
   WHERE soortnaam = '%s'
     ...
     AND ABS(sd_waterlichaam - %f) < 1e-4
  ...
  if (nrow(sims) == 0) {
    ...
    data.frame(
      ...
      sd_waterlichaam = count_sd_waterlichaam,
  ...
  #voorlopig niet nodig als we telkens met een heel lage power vertrekken
