...
# gelijkaardig aan functie sim_3p, maar deze functie is voor 1 specifiek waterlichaam
sim_3p_waterlichaam <- function(
  ...
  replicate(n_sim, {
    ...
    expand.grid(
      ...
      mutate(
        ...
        eta = count_intercept + intercept_waterlichaam +
          rnorm(n_datum, sd = count_sd_datum)[.data$datum] +
          rnorm(n_datum * n_vispunt, sd = count_sd_vispunt)[.data$vispunt],
    ...
    sim_data |>
      count(.data$datum, .data$vispunt, .data$type) |>
      glmmTMB(
        formula = n ~ type + + (1 | datum) + (1 | vispunt),
